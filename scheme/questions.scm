...
(define (cons-all first rest) (map (lambda (x) (cons first x)) rest))
...
(define (seconds pairs)
     (if
       (null? pairs)
         '()
        (cons (car (cdar pairs)) (seconds (cdr pairs)))))



(define (zip pairs)
  (list (firsts pairs) (seconds pairs)  ))
...
(define (enumerate s)
  ; BEGIN PROBLEM 17
  (define (indexed-pairs index s)
          (if (null? s)
              s
              (cons (cons index (cons (car s) nil))
              (indexed-pairs (+ index 1) (cdr s)))
              ))
          (indexed-pairs 0 s)
