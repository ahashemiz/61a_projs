...
def announce_highest(who, previous_high=0, previous_score=0):
    ...
    # END PROBLEM 7
    def score_check(score0, score1):

        if who == 0:      #Useful way of reducing code, to implement in play func later
            player_score = score0

        elif who == 1:
            player_score = score1

        score_gain = player_score - previous_score
        new_high = previous_high

        if score_gain > new_high:
            new_high = score_gain
            point = "point"
            if score_gain > 1:
                point += "s"
            print("{0} {1}! That's the biggest gain yet for Player {2}".format(score_gain, point, who))
#New way to handle prints, change other prints later
        return announce_highest(who, new_high, player_score)
    return score_check
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
    ...
    k = 1
    while k <= 10:
        avg_value = make_averaged(roll_dice, num_samples)(k, dice)
