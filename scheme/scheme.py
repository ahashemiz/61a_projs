...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        try:        #python_args = list(map(self.fn, *args))
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        new_pair = Pair(target.second, expressions.second)

        env.define(target.first, do_lambda_form(new_pair, env))
        return target.first
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    while expressions.second:
        # if expressions.first == 0:
        #     print('iterated')
        # if not scheme_eval(expressions.first, env):
        #     if expressions.first != 0:
        #         return '#f'
        if scheme_eval(expressions.first, env) or type(expressions.first) == int:
            #expressions.first == 0 or
            #scheme_eval(expressions.first, env)
            expressions = expressions.second
        else:
            return '#f'
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        symbol_pair = bindings.first.first
        val_pair = bindings.first.second.first

        symbols = Pair(symbol_pair, symbols)
        vals = Pair(scheme_eval(val_pair, env), vals)
