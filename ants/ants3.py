    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 9
                an_ant = self.ant
                if an_ant.can_contain(insect):
                    an_ant.contain_ant(insect)
                elif insect.can_contain(self.ant):
                    insect.contain_ant(an_ant)
                    self.ant = insect
                else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        increment = 0
        spot = self.place
        while spot is not hive:

            if spot.bees and self.min_range <= increment <= self.max_range:
                increment += 1
...
class HungryAnt(Ant):
    ...
    def __init__(self, armor=1):
        ...
        # 'default 0, ready to eat'
        # "digesting" 'decrement time to digest' ' set disgesting to time_to_digest when eats'
        # 'when eats, start off at time to digest = 3'
        # 'decrement each turn'
        # END Problem 6
    ...
    def eat_bee(self, bee):
        # BEGIN Problem 6
        if self.place.bees:
            # targeted_bee = random_or_none(self.place.bees)
    ...
    def action(self, colony):
        ...
        if self.digesting != 0:
            ...
            targeted_bee = random_or_none(self.place.bees)
            self.eat_bee(targeted_bee)
        # "checkig digesting, if digesting > 0, decrement digesting counter; else if 0
        # 'otherwise, self.eat_bee'
        # END Problem 6
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        self.true_queen = not QueenAnt.queen_placed
        ...
        while place:
            if place.ant:
                if not place.ant.damage_doubled:
                    doubled_ants.append(place.ant)
                    for a in doubled_ants:
                        if a.container:
                            if a.ant:
                                a.damage, a.ant.damage = a.damage *2, a.ant.damage*2
                                a.damage_doubled, a.ant.damage_doubled = True
                                doubled_ants.append(place.a.ant)
                        else:
                            a.damage *= 2
                            a.damage_doubled = True
            place = place.exit
        # END Problem 13
